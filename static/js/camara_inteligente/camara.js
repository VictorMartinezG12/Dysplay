 ...
 * foto del objeto enfocado y la envía al backend para reconocerlo (Google
 * Cloud Vision) y generar una frase de práctica, grava la pronunciación del
 * estudiante (codificada como WAV real para Azure Speech) y muestra el
 * resultado con las monedas obtenidas.
 */
...
(() => {
     ...
     * video al elemento `#video-camara`.
     * @returns {Promise<void>}
     */
    async function iniciarCamara() {
        try {
            videoStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } });
            document.getElementById('video-camara').srcObject = videoStream;
        } catch (err) {
            console.error(err);
